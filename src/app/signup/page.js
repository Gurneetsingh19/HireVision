...
import { BrainCircuit, Mail, Lock, ArrowRight, Code, Globe, Sparkles, CheckCircle2 } from 'lucide-react';
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/signup`, {
        ...
        body: JSON.stringify({
          email,
          password,
          role: loginType, // candidate selected -> "candidate", company selected -> "company"
        }),
      });
      ...
      if (data.success) {
        // If signup success: show success message and redirect to /login
        setSuccess(data.message || "Signup successful! Redirecting to login...");
        setTimeout(() => {
          window.location.href = "/login";
        }, 1500);
      } else {
        // If signup fails: show backend error message
    ...
    <div className="min-h-screen flex bg-[#fafbfe]">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 relative overflow-hidden">
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-extrabold text-slate-800 tracking-tight mb-2">Create your account</h1>
            <p className="text-slate-500 font-semibold text-sm">Please enter your details to register.</p>
          </div>
          ...
          <GlassCard className="!p-8 border border-white/80 shadow-2xl rounded-3xl bg-white/70">
            ...
            {/* Success Message */}
            {success && (
              <div className="bg-emerald-50 border border-emerald-200 text-emerald-600 px-4.5 py-3 rounded-2xl text-xs font-semibold mb-6 flex items-center gap-2">
            ...
            <p className="mt-8 text-center text-sm text-slate-500 font-semibold">
              Already have an account? <Link href="/login" className="font-bold text-blue-600 hover:underline hover:text-indigo-600 transition-colors">Log in</Link>
            </p>
          </GlassCard>
        </div>
      </div>
